...
class CtdetLossVid(torch.nn.Module):
    def __init__(self, opt):
        super(CtdetLossVid, self).__init__()
    ...
    def forward(self, outputs, batch):
        ...
        for s in range(opt.num_stacks):
            ...
            # cv2.imwrite('/store/datasets/UA-Detrac/exp/tensors/VID_HM/HM_GT.jpg', write_hmgt)
            ...
            # cv2.imwrite('/store/datasets/UA-Detrac/exp/tensors/VID_HM/HM_RE.jpg', write_hmre)
...
class CtdetTrainerVid(BaseTrainer):
    def __init__(self, opt, model, optimizer=None):
        super(CtdetTrainerVid, self).__init__(opt, model, optimizer=optimizer)

    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'wh_loss', 'off_loss']
        loss = CtdetLossVid(opt)
