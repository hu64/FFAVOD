...
import cv2
from models.networks.large_hourglass_SpotNet2 import segmentation_module
from models.networks.large_hourglass_SpotNet2 import make_saliency_layer
from models.networks.large_hourglass_vid import FusionModule
import os
...
class exkp(nn.Module):
    def __init__(
        ...
        for head in heads.keys():
            ...
            elif 'seg' in head: # hughes
                module = nn.ModuleList([
                    # make_saliency_layer(curr_dim, heads['hm']) for _ in range(nstack)
                    segmentation_module(curr_dim, heads['hm']) for _ in range(nstack)
                ])
                self.__setattr__(head, module)
            else:
                module = nn.ModuleList([
...
class HourglassNetSpotNetVid(exkp):
    def __init__(self, heads, num_stacks=2, nbr_frames=5):
        n       = 5
        dims = [256, 256, 384, 384, 384, 512]
        modules = [2, 2, 2, 2, 2, 4]

        super(HourglassNetSpotNetVid, self).__init__(
...
def get_large_hourglass_net_SpotNetVid(num_layers, heads, head_conv, nbr_frames):
  model = HourglassNetSpotNetVid(heads, 2, nbr_frames)
  return model
