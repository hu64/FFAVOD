...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('task', default='ctdet',
                             help='ctdet | ddd | multi_pose | exdet | ctdetSpotNet2 | ctdetWipeNet |ctdetMultiSpot | ctdetVid | ctdetSpotNetVid')
    ...
    self.parser.add_argument('--arch', default='dla_34',
                             help='model architecture. Currently tested'
                                  ...
                                  'dlav0_34 | dla_34 | hourglass | hourglassSpotnet2 | hourglassWipeNet')
    ...
    self.parser.add_argument('--nbr_frames', type=int, default=5,
                             help='Nbr frames used for VID variations.')
        # hughes
    ...
    self.parser.add_argument('--tlbr_weight', type=float, default=0.1,
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'exdet':
      ...
      opt.heads = {'hm': opt.num_classes,
                   'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes,
                   'seg': 1}
    elif opt.task == 'ctdetWipeNet':
      opt.heads = {'hm': opt.num_classes,
                   'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes,
                   'tlbr': opt.nbr_points*2}
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
    ...
    default_dataset_info = {
      ...
      #             'dataset': 'ua-detrac-1-on-10'},
         'ctdet': {'default_resolution': [512, 512], 'num_classes': 1,
                   'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                   'dataset': 'uav'},
        'ctdetSpotNet2': {'default_resolution': [512, 512], 'num_classes': 1,
                  'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                  'dataset': 'uav'},
        'ctdetWipeNet': {'default_resolution': [512, 512], 'num_classes': 10,
                          'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                          'dataset': 'uadetrac1on10_b'},
        'ctdetVid': {'default_resolution': [512, 512], 'num_classes': 1,
                  'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                  'dataset': 'uadetrac1on10_b'},
        'ctdetSpotNetVid': {'default_resolution': [512, 512], 'num_classes': 1,
                     'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                     'dataset': 'uadetrac1on10_b'},
